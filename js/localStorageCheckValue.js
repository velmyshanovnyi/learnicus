// JavaScript Document
// Для "options.html" та "popup.html" бо вони юзають однакову логіку
// localStorageCheckValue.js
// learnicus
var ThisFileName    = 'localStorageCheckValue'; // назва цього файла.
var ThisFileNameExt = 'js';                     // тип цього файла. 
localStorage.setItem('learnicusCheckboxPrefix', 'learnicusCheckbox_'); // закидаємо змінну "gmmCheckboxPrefix" в ЛС щоб можна було її брати в усіх файлах


console.log(ThisFileName +': Почали читати '+ThisFileName);
console.log(ThisFileName +': У нас зараз: learnicusCheckboxPrefix = '+localStorage.learnicusCheckboxPrefix+' (пишемо в консолі: localStorage.learnicusCheckboxPrefix)');
...
function getStorage(key_prefix) {
    ...
    if (window.localStorage) {
        // (use localStorage; Використовуємо ЛС;):
        return {
        ...
        // (use document.cookie; Використовуємо кукі, якщо не вийшло з ЛС;):
        return {
            set: function(id, data) {
...
jQuery(function($) {
    ...
    var $inputs = $('input.filter'), $checkall = $('input.checkAll'), $storedData = getStorage(localStorage.learnicusCheckboxPrefix); // як префікс до всіх чекбоксів
...
console.log('FULL LOAD|js/localStorageCheckValue.js    |=OK!');